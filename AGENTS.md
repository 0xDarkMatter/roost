...
| `roost widget` | Self-contained HTML fleet dashboard on stdout, for Claude Code's `show_widget` tool: header (ring per profile, current recommendation, Claude Status panel) above per-profile capacity cards. `--max-kb` sets the byte budget (default 28) |
...
32. **`roost widget` output must stay self-contained and under its byte budget.** The page is rendered inline by Claude Code's `show_widget` tool behind a strict CSP: no CDN script or stylesheet, no webfont, no `fetch`/XHR, no `<script src>`/`<link>`, no outbound request of any kind — `widget.py` ships one inline `<style>` block and plain markup, deliberately no `<script>` tag at all. Never introduce a `<script>` tag, an external asset reference, `Arial` anywhere in the font stack, or a native `alert`/`confirm`/`prompt` call into this renderer — the CSP silently breaks the first three, and the last is a standing ban on this machine with no carve-out for a fragment this small. See the `widget.py` module docstring.

33. **The byte budget sheds DETAIL before it sheds a profile.** `render_widget()` steps down `_DETAIL_LEVELS` (`full` → `compact` → `minimal`), dropping the alternate bar view and then the per-service incident calendars, and only removes a profile card if the page still doesn't fit. This ordering is the whole point: the drop path removes the **least-recently-probed** profile, which is precisely the account an operator is least likely to notice missing — and a fleet overview that silently omits an account is worse than one missing a nicety. This was a real shipped bug: a four-profile fleet rendered three cards, and the `warnings.warn` notice went to stderr where a redirect swallowed it. Two traps if you touch this: (a) bars are the CSS **default**, so dropping the bar markup without also applying the `rw-sq-only` class renders cards with *no gauges at all*; (b) the toggle must be omitted at reduced detail or one of its two positions shows nothing. Both are covered by tests. See `widget.py::render_widget` and `test_detail_is_shed_before_any_profile_is_dropped`.

34. **The Bars/Grid toggle is CSS-only, and that is load-bearing.** It is a hidden `<input type="checkbox">` plus forward sibling selectors — no JavaScript. The page's "no `<script>` at all" property is what makes "this renderer cannot reach for `alert`/`confirm`/`prompt`" **structurally** true rather than a convention someone has to remember. A click handler would trade that guarantee away for a control CSS already expresses. The checkbox must precede both the header (which holds its `<label>`) and the card grid (which it restyles), because CSS only selects **forward** siblings — moving it breaks the toggle silently, with no error. Same trap in the status panel: the service rows and `.rw-strips` must stay siblings for the hover-swap rules, so the side-by-side columns are made with grid *placement*, not by wrapping the rows in a container. See `widget.py::_render` and the `.rw-modechk` / `.rw-svc:hover` rules.

35. **The square-grid gauges are drawn with gradients on whole-pixel boundaries.** Each 10×10 block is ONE element painted with three stacked gradients (two repeating ones for the card-coloured gaps, one linear for the bottom-up fill), not 100 tags — real elements would be ~1,200 across four cards and would blow the byte budget on their own. Every stop is an integer pixel and the fill is quantised to whole rows, because an earlier build used percentage stops to make the block fluid-width and every cell edge landed on a fractional pixel: the browser antialiased all 200 of them and the grid rendered visibly soft with uneven gaps. **The CSS is derived from `_CELL_PX` / `_GAP_PX` / `_PERIOD_PX` / `_GRID_PX` rather than hand-written, and the fill maths uses the same constants** — a stop edited in one place and not the other misaligns the fill boundary. Do not convert these back to percentages to chase a fluid layout. See `widget.py::_gauge_column` and the `.rw-sqgrid` rule.

36. **Incident history is "incidents", never "uptime", and the window is read from the data.** Statuspage's public API exposes incident *records*; the uptime percentage on the status page is computed from data it does not expose. A day with nothing reported is "no incident reported" — a weaker claim than "100% up", and labelling it otherwise would put a number on screen that Anthropic never published. The endpoint also returns only the ~50 most recent incidents, so the real span is whatever that reaches back to (~29 days at the time of writing, not the 90 the status page shows): render the label from `history_days` in the payload, never a hardcoded window. See `widget.py::_incident_window_text` and `platform_status.py`.

37. **Don't put exec outcomes on a profile card without a recency window.** Exec failure rate and median exec duration were both computed, shipped, and removed. `rc` is the **child's** exit code — `roost exec -- claude ...` propagates whatever the child returned, so a bad prompt, a Ctrl+C, or a deliberate failure test all read as *the profile* failing, in alert-red, on a card whose entire job is answering "is this profile healthy?". The samples are also tiny and never expire: one profile's "Failed 100%" was six runs inside three minutes on a single day months earlier, and it would have read 100% forever. Median had the same defect from the other side — it timed the command, not the account, averaging a 47ms `env` with a 133s claude session. If exec outcomes are ever worth surfacing they need a recency window *and* a way to separate "the profile was unusable" from "the command exited non-zero". Absent beats misleading. The surviving chips are `Picks` (roost's own dispatch counts, thousands of samples) and `Full in` (the usage-log burn-rate projection). See `widget.py::_render_stats`.

38. **The dashboard's recommendation must stay read-only.** The header names the profile `pick` would return, which means rendering it runs the pick algorithm. `pick()` itself is pure — the side effects (`append_pick_log`, `write_last_pick`) live in the CLI layer — so `cli._recommended_profile` calls the **function**, never the command. Routing it through the `pick` command path would append to `picks.log` and rewrite `last-pick.json`, meaning that merely *looking* at the fleet would change which profile the next dispatch gets. This is rule 21's guarantee applied to a new surface. Stickiness is also disabled for that query on purpose: a sticky answer reports whatever was picked last rather than what is healthiest now, which is the opposite of what an overview is for. A test asserts neither file is created. See `cli.py::_recommended_profile`.

39. **The status panel is branded "Claude Status", not "Anthropic".** It renders data from `status.claude.com`, whose components are all Claude-branded — "Anthropic" is the company, not the product, and the word must not appear in the **rendered output** (a test asserts it). Source comments naming Anthropic as the provider are fine and accurate. The Claude mark lives in the icon sprite and is referenced twice via `<use href="#i-claude">`; the path is ~1.8 KB, so a second inline copy would cost more than a whole profile card. Every stat icon works the same way — a webfont is not an option under the CSP, and repeating paths across twenty chips would cost more than the cards. See `widget.py::_claude_mark` and `_ICON_SPRITE`.
